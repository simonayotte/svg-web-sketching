...
describe('ContinueDrawingService', () => {
    ...
    it('#loadSavedDrawing() should call #setCanvasColor() of the store', () => {
        ...
        const canvasColorFromJSON = new Color(
            drawingJSON.canvasColor[0],
            drawingJSON.canvasColor[1],
            drawingJSON.canvasColor[2],
            drawingJSON.canvasColor[3],
        );
        service.loadSavedDrawing();
        expect(store.setCanvasColor).toHaveBeenCalledWith(canvasColorFromJSON);
    });
